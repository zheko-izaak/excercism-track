...
export function remainingMinutesInOven(actualMinutesInOven) {
  ...
  if (actualMinutesInOven < EXPECTED_MINUTES_IN_OVEN) {
    result = EXPECTED_MINUTES_IN_OVEN - actualMinutesInOven;
  }
  return result;
}
...
export function preparationTimeInMinutes(numberOfLayers) {
  ...
  if (numberOfLayers > 0) {
    result = PREPARATION_MINUTES_PER_LAYER * numberOfLayers;
  }
  return result;
}
...
export function totalTimeInMinutes(numberOfLayers, actualMinutesInOven) {
    ...
    if (actualMinutesInOven > 0) {
      result = preparationTimeInMinutes(numberOfLayers) + actualMinutesInOven;  
    }
    else {
      result = preparationTimeInMinutes(numberOfLayers);
    }
